...
interface AdminUser {
  email: string;
  role: "superadmin" | "admin" | "user" | "seller" | "enroller";
  name: string;
}
    ...
    <div className="min-h-screen bg-[#0F1117]">
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-64 bg-[#1A1D27] border-r border-white/5 flex flex-col transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto">
          {user.role === "enroller" ? (
            <>
              <p className="px-3 py-2 text-[10px] font-semibold text-white/20 uppercase tracking-wider">Enrolment</p>
              {ENROLLER_NAV_ITEMS.map((item) => {
